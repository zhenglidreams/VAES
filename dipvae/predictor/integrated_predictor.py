...
class IntegratedPredictor(pl.LightningModule):
    ...
    def forward(self, batch) -> torch.Tensor:
        ...
        z, _ = self.vae.encode(one_hot.squeeze(1))
        # Add explicit information on depth of network
        n_layers = n_layers.type_as(z).view(z.shape[0], 1)
...
if __name__ == "__main__":
    ...
    stgs.VAE_HPARAMS['weights_path'] = Path("H:/gvae_exps/test_predictor/vae_wts/weights.pt")
    ...
    logger = TensorBoardLogger("H:/gvae_exps/test_predictor/integrated_pred_ckpts")
    trainer = pl.Trainer(
