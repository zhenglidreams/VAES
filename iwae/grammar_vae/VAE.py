...
class NA_VAE(pl.LightningModule):
    ...
    def forward(self, x,lengths):
        ...
        z=z.reshape(-1, self.latent_sz-1)
        length=lengths.transpose(0, 1)
        length_final=length.repeat(self.num_samples, 1, 1).permute(1, 0, 2)
        length_final=length_final.reshape(-1,1)
        z = torch.cat((z, length_final), dim=1)
        z=z.view(-1,self.num_samples,self.latent_sz)
    ...
    def loss_function(self, x_decoded_mean, x_true, mu, logvar,B):  # K: bce(target, output)
    ...
    def training_step(self, batch, batch_idx):
        ...
        x_recon, mu, logvar,B=self.forward(x,lengths)
