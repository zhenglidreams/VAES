...
class NA_VAE(pl.LightningModule):
    ...
    def decode(self, z):
        ...
        h_rnn = self.dec_rnn_do(self.dec_rnn_fc_1(z))
        h_rnn = h_rnn.unsqueeze(1)
        h_rnn = h_rnn.repeat((1, self.max_len, 1)); 
        self.check_sz(h_rnn, (self.max_len, self.latent_sz))
        out_rnn, _ = self.dec_gru(h_rnn); 
    ...
    def loss_function(self, x_decoded_mean, x_true, mu, logvar,q,batch_id_x):
        ...
        cont_curr = (self.cont_max - self.cont_min) * self.iter/ float(self.cont_iter) + self.cont_min
